...
## 2026-08-28 — React demo UI

- Options: Swagger-only / separate SPA repo / SPA in this repo.
- Choice: **Vite + React + TypeScript** in `frontend/`, Compose service `questkeep-web` on host **8080** (nginx: SPA + `/api` + Swagger proxy). Java listens only inside the Compose network.
- Why: owner asked for a clickable demo. Same origin avoids extra CORS. No new backend endpoints.

## 2026-08-28 — GitHub Actions CI

- Options: no CI / tests-only workflow / tests + CodeQL/deploy.
- Choice: **tests + frontend build** (`.github/workflows/ci.yml`) on `main`, pull requests, and `workflow_dispatch`.
- Why: Testcontainers needs Docker; GitHub-hosted `ubuntu-latest` provides it. Same Maven Wrapper + JDK 21 as the Dockerfile. Frontend job: Node 22, `npm ci && npm run build`. No deploy, no security scanning (still out of scope).
